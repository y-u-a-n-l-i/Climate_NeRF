DATA_ROOT=/hdd/datasets/TanksAndTempleBG/Truck
...
# train
python train.py --config configs/Truck.txt --exp_name truck \
    --root_dir $DATA_ROOT --sem_conf_path $SEM_CONF --sem_ckpt_path $SEM_CKPT

# Testing view
python render.py --config configs/Truck.txt --exp_name truck \
    --root_dir $DATA_ROOT --sem_conf_path $SEM_CONF --sem_ckpt_path $SEM_CKPT \
    --weight_path ckpts/tnt/truck/epoch=79_slim.ckpt \
    --render_depth_raw

# Smog
python render.py --config configs/Truck.txt --exp_name truck-smog \
    --root_dir $DATA_ROOT --chunk_size -1 \
    --weight_path ckpts/tnt/truck/epoch=79_slim.ckpt \
    --depth_path results/tnt/truck/depth_raw.npy \
